...
import { plugin, executeTextResponse } from "../src/vue";
import type { ToolResult, ToolSample, ToolPlugin } from "gui-chat-protocol/vue";
import type { TextResponseData, TextResponseArgs } from "../src/core/types";
...
const result = ref<ToolResult<TextResponseData>>({
...
const executeSample = async (sample: ToolSample) => {
  const args = sample.args as unknown as TextResponseArgs;
  const execResult = await executeTextResponse({} as any, args);
  result.value = {
    ...result.value,
    ...execResult,
    uuid: `demo-${Date.now()}`,
...
const handleUpdate = (updated: ToolResult<TextResponseData>) => {
